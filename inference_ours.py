...
def get_clipgrounding_output(model, image, reference, caption, box_threshold, text_threshold=None, with_logits=True, cpu_only=False, token_spans=None):
    ...
    with torch.no_grad():
        outputs = model(image[None], references=[reference], captions=[caption])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reference_image", "-r", type=str, default=None, help="reference image path")
    ...
    # load image
    image_pil, image = load_image(image_path)
    ref_image = Image.open(reference_image).convert("RGB") if reference_image is not None else Image.new("RGB", (224, 224), (0, 0, 0))
    # load model
    ...
    if reference_image is not None:
        text_prompt +=  " . REF OBJECT"
    
    # run model
    boxes_filt, pred_phrases = get_clipgrounding_output(
        model, image, ref_image, text_prompt, box_threshold, text_threshold, cpu_only=args.cpu_only, token_spans=token_spans
    )
